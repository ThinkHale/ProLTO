...
9. Truck geometry moved from runtime Three.js primitives to parametric Blender models exported as articulated glTF binaries. Bodywork is lofted and subdivided rather than beveled boxes, structural members are extruded profiles, and recognition hardware such as mast chains, cylinders with chrome rods, bolt circles, guard tubing, tread plate, labels, displays and control hardware is modeled explicitly.
10. Desktop eye points are derived from anthropometry against each truck's own floor or seat height. WebXR uses a separate `rig_xrOrigin` at the operator floor, allowing the headset's tracked height to supply physical eye height without a second authored offset.
