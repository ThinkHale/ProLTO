  ...
  return (
    <aside className="evaluator-panel">
      <section className="candidate-summary panel-section">
        <div className="candidate-line"><div className="avatar">JL</div><div><input value={candidateName} onChange={(e) => onCandidateChange(e.target.value)} aria-label="Candidate name"/><span>Candidate assessment</span></div><i>Live</i></div>
        <p>{state.profile.manufacturer} {state.profile.model}</p>
        <strong>{state.family.classLabel} {state.family.label}</strong>
        <div className="progress-label"><span>Assessment progress</span><b>{Math.round((completed / checklist.length) * 100)}%</b></div>
        <div className="progress"><span style={{ width: `${(completed / checklist.length) * 100}%` }} /></div>
      </section>
      ...
      <section className="panel-section checklist-section">
        ...
        {checklist.map(([id, label]) => <button key={id} onClick={() => onToggleCheck(id)}>{state.checks[id] ? <CheckCircle2 size={17} /> : <Circle size={17} />}<span>{label}</span></button>)}
      </section>
